...
let EXPERIMENT = Nof1;
    ...
    let experiment_configuration_function = (writer) => { return {
        ...
        post_questionnaire: [
            writer.free_text("Name","What's your name?"),
            writer.free_text("Age","How old are you?"),
            writer.alternatives("Status","What is your current working status?",
            ...
            writer.free_text("Experience","How many years of working experience in software industry to you have?"),
            writer.free_text("LoC","How many lines of code do you think you write each day on average?"),
        ],
        ...
        measurement: EXPERIMENT.Time_to_finish(EXPERIMENT.text_input),

        task_configuration:    (t) => {
            let changes = parseInt(t.treatment_combination[1].value);
            // let diff_group = parseInt(t.treatment_combination[2].value);
            // @ts-ignore
            let possible_graphs = EXPERIMENT.graph_dictionary()[5][changes];
