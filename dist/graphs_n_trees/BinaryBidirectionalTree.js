...
export class BinaryBidirectionalTree {
    ...
    clone() {
        let left_child = (this.left != null) ? this.left.clone() : this.left;
    ...
    set_all_operator_values(operator_value) {
        let arr = this.as_in_order_array();
        arr = arr.filter((e) => !e.is_leave());
        arr.forEach(e => e.content = operator_value);
    }
    set_all_leave_values(leave_value) {
        let arr = this.as_in_order_array();
        arr = arr.filter((e) => e.is_leave());
    ...
    call_by_value_order_writer(arr) {
        if (this.is_leave()) {
            arr.push(this);
            return;
        }
